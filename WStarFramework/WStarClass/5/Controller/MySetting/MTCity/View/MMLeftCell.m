...
- (void)setItem:(MMItem *)item {
    ...
    self.backgroundColor = item.isSelected?[UIColor colorWithHexString:SelectedBGColor alpha:1]:[UIColor colorWithHexString:UnselectedBGColor alpha:1];
    self.infoLabel.textColor = item.isSelected?[UIColor colorWithHexString:titleSelectedColor alpha:1]:[UIColor blackColor];
}
- (void)layoutSubviews {
    [super layoutSubviews];
    self.infoLabel.frame = CGRectMake(LeftCellHorizontalMargin, 0, self.width - 2 *LeftCellHorizontalMargin, self.height);
    self.bottomLine.frame = CGRectMake(0, self.height - 1.0/Kscale , self.width, 1.0/Kscale);
}
