...
- (id)initWithFrame:(CGRect)frame titleName:(NSString *)title {
    ...
    if (self) {
        ...
        self.titleLabel.frame = CGRectMake(DropDownBoxTitleHorizontalToLeft, 0 ,self.arrow.x - DropDownBoxTitleHorizontalToArrow - DropDownBoxTitleHorizontalToLeft  , self.height);
        ...
        self.line.frame = CGRectMake(self.arrow.x + ArrowToRight - 1.0/Kscale , 0, 1.0/Kscale, self.height);
...
- (void)updateTitleState:(BOOL)isSelected {
    if (isSelected) {
        self.titleLabel.textColor = [UIColor colorWithHexString:titleSelectedColor alpha:1];
